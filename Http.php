...
//    public static function decode($recv_buffer, TcpConnection $connection)
    public static function decode($recv_buffer, $remote_address)
    {
        // 初始化
...
//        HttpCache::$instance = new HttpCache();
        ...
        $pos = strrpos($remote_address, ':');
        $_SERVER['REMOTE_ADDR'] = $pos ? substr($remote_address, 0, $pos) : '';
        $_SERVER['REMOTE_PORT'] = $remote_address ? substr(strrchr($remote_address, ':'), 1) : '';
...
//        print_r($header_data);exit;
        $header = "HTTP/1.1 ";
        //处理header返回码
        if(preg_match('/Status: (.*)/', $header_data[0], $match)) {
        ...
        Array
        (
            [0] => X-Powered-By: PHP/5.6.17
            [1] => Content-type: text/html; charset=UTF-8
        )
        错误则php-fpm返回如下
        Array
        (
            [0] => Primary script unknown
Status: 404 Not Found
            [1] => X-Powered-By: PHP/5.6.17
